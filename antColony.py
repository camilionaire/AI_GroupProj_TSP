...
def ant_data(path_data, total, my_table, traversal_list, path_travelled, i, ant_results, destination):
    for j in range(0, len(path_data)):  # should run five times for first table
        ...
        duplicate_checked = False  # check if there are duplicate values
        for k in path_data[:]:
            print('k is', k)  # get each value from the row
            if path_data.count(k) == 1:
                start_at = -1
                path_index = path_data.index(k, start_at + 1)  # get the index for each value
            elif path_data.count(k) == 2:
                if duplicate_checked == False:
                    start_at = -1
                    path_index = path_data.index(k, start_at + 1)
                    duplicate_checked = True
                elif duplicate_checked == True:
                    start_at = -1
                    path_index = path_data.index(k, start_at + 2)  # get the index for each value

            print(path_index)
            if (k < min) and (path_index not in path_travelled):
                min = k
        ...
        if path_data.count(min) == 1:
            start_at = -1
            path_travelled.append(path_data.index(min))
            print('current nodes visited', path_travelled)
            path_data[:] = list(my_table[path_data.index(min)])
        elif path_data.count(min) > 1:
            if duplicate_checked == False:
                start_at = -1
...
def aco_main(my_table):
    ...
    while (current_iter < max_iter):
        for i in colony_list:
            ...
            path_data = list(my_table[destination])  # select the correct row from the imported table
